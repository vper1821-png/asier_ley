...
$navItems = [
    ...
    ['id' => 'certification', 'label' => 'Certificación', 'icon' => 'certification'],
...
// ✅ FIX: eliminada la Closure $scoreColor que rompía pages/certification.php:106
// (no se usaba en este archivo y colisionaba con la variable del scope global)
...
<aside class="flex flex-col bg-bg-base border-r border-border-theme transition-all duration-300 flex-shrink-0 <?= $collapsed ? 'w-16' : 'w-56' ?> hidden md:flex">
    ...
    <div class="px-4 py-3 border-b border-border-theme tour-pending-ring relative group">
        ...
        <div class="absolute left-full top-0 ml-2 w-72 bg-bg-panel border border-border-theme rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 p-4">
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center justify-between gap-2 p-2 rounded-lg <?= ($task['done'] ?? false) ? 'bg-emerald-500/5' : 'bg-bg-surface/30' ?>">
                    <div class="flex items-center gap-2 min-w-0">
                        <span class="w-1.5 h-1.5 rounded-full <?= ($task['done'] ?? false) ? 'bg-emerald-400' : 'bg-yellow-400' ?> flex-shrink-0"></span>
                        <?php if (empty($task['done'])): ?>
                        <!-- ✅ FIX: fallback para $task['link'] que no siempre viene del backend -->
                        <a href="<?= h($task['link'] ?? '/compliance') ?>" class="text-[10px] text-text-body truncate hover:text-primary-300 transition-colors">
                            <?= h($task['label'] ?? 'Tarea') ?>
                        </a>
                        <?php else: ?>
                        <span class="text-[10px] text-text-body truncate"><?= h($task['label'] ?? 'Tarea') ?></span>
                        <?php endif; ?>
                    </div>
                    <?php if (!empty($task['done'])): ?>
    ...
    <div class="px-3 py-3 border-t border-border-theme space-y-1.5">
        <?php /* Tour guiado deshabilitado temporalmente
            ...
            <div id="notif-panel" class="hidden absolute bottom-full left-0 mb-2 w-72 max-h-80 overflow-y-auto rounded-xl border border-border-theme bg-bg-panel shadow-xl z-50 scrollbar-custom">
                <div class="px-3 py-2 border-b border-border-theme flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button onclick="clearAllNotifications()" class="text-[10px] text-red-400 hover:text-red-300" title="Borrar todas las notificaciones">Borrar todas</button>
                    </div>
                </div>
...
async function clearAllNotifications() {
    ...
    try {
        await fetch('/api-proxy.php?path=/api/notifications/clear-all', { method: 'POST' });
        loadNotifications();
        updateNotifBadge();
    } catch (e) { /* noop */ }
}
