...
$typeIcons = [
    ...
    'supresion' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 7V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>',
            ...
            <div class="flex-1 min-h-0 bg-bg-panel/80 border border-border-theme rounded-2xl overflow-hidden backdrop-blur-md shadow-theme-sm flex flex-col">
                ...
                <div class="flex-1 overflow-y-auto p-2.5 space-y-2 scrollbar-custom" id="arco-inbox-items">
                    ...
                    <div data-arco-item
                        ...
                        <div id="arco-resp-<?= h($rid) ?>" class="hidden px-4 pb-4 pt-3 border-t border-white/[0.04] bg-bg-surface/20">
                            <form method="POST" class="space-y-3">
                                <input type="hidden" name="request_id" value="<?= h($rid) ?>">
                                <div class="grid grid-cols-1 md:grid-cols-[160px_1fr] gap-3">
                                    <div>
                                        <label class="block text-[9px] font-medium text-text-subtle uppercase tracking-wider mb-1.5">Estado</label>
                                    ...
                                    <div>
                                        <label class="block text-[9px] font-medium text-text-subtle uppercase tracking-wider mb-1.5">Respuesta al titular</label>
                                        <textarea name="response" rows="3" placeholder="Escribe la respuesta que complementará el documento formal..." class="input-premium w-full resize-none text-[12px] leading-relaxed"><?= h($r['response'] ?? '') ?></textarea>
                                    </div>
                                </div>
                                <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-1">
                                    <p class="text-[10px] text-text-subtle leading-relaxed">
                                        Descarga el documento formal PDF. La respuesta quedará registrada junto a la solicitud ARCO.
                                    </p>
                                    <div class="flex items-center gap-2">
                                        <a href="/api/arco/requests/<?= h($rid) ?>/document?token=<?= h($token) ?>" target="_blank"
                                            class="px-3 py-2 rounded-lg text-[11px] font-medium bg-white/[0.05] border border-white/[0.08] text-text-muted hover:text-white hover:bg-white/[0.08] transition-all flex items-center gap-1.5 whitespace-nowrap">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                            Documento PDF
                                        </a>
                                        <?php if (($r['tipo'] ?? $r['type'] ?? '') === 'portabilidad'): ?>
                                        <a href="/api/arco/requests/export-portabilidad?requestId=<?= h($rid) ?>&format=json&token=<?= h($token) ?>"
                                        ...
                                        <a href="/api/arco/requests/export-portabilidad?requestId=<?= h($rid) ?>&format=csv&token=<?= h($token) ?>"
                                            class="px-3 py-2 rounded-lg text-[11px] font-medium bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20 transition-all flex items-center gap-1.5 whitespace-nowrap">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                            CSV
                                        </a>
                                        <?php endif; ?>
                                        <button type="submit" name="update_request" value="1" class="px-4 py-2 rounded-lg text-[11px] font-medium bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:from-blue-500 hover:to-indigo-500 transition-all shadow-theme-sm flex items-center gap-1.5">
