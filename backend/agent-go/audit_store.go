...
// Stores host events, query logs, connection
// events, and file events so they survive agent restarts.
// ──────────────────────────────────────────────
...
// ─── NUEVO: StoreFileEvent guarda un archivo detectado ───
func (s *AuditStore) StoreFileEvent(detected *DetectedFile) {
    if s == nil || s.db == nil {
        return
    }
    s.mu.Lock()
    ...
    personalDataJSON, _ := json.Marshal(detected.PersonalData)
    _, err := s.db.Exec(
        `INSERT INTO file_events (timestamp, path, file_type, hash, row_count, personal_data, sensitive, agent_id, hostname, reported)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        ...
        func() int { if detected.Sensitive { return 1 }; return 0 }(),
        detected.AgentID,
        detected.Hostname,
        1,
    )
    if err != nil {
        logMsg("AuditStore: insert file_event failed: %v", err)
    }
}
...
func (s *AuditStore) GetStats() map[string]interface{} {
    ...
    var totalHost, totalQuery, totalConn, totalWin, totalFile int
    ...
    s.db.QueryRow(`SELECT COUNT(*) FROM file_events`).Scan(&totalFile)

    stats["host_events"] = totalHost
    stats["query_logs"] = totalQuery
    stats["connection_events"] = totalConn
    stats["windows_events"] = totalWin
    stats["file_events"] = totalFile
    stats["total"] = totalHost + totalQuery + totalConn + totalWin + totalFile
...
func (s *AuditStore) CleanupOld(retentionDays int) {
    ...
    tables := []string{"host_events", "query_logs", "connection_events", "windows_events", "file_events"}
